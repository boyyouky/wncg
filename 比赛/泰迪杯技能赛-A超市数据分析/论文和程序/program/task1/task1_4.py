...
for i in range(18):
    week_money[i][0]= i+1
for i in range(len(days_money)):
    days_money[i][0]=day_to_week(days_money[i][0])
for i in range(len(days_money)):
    weekflag = days_money[i][0]-1
    if(days_money[i][0]==week_money[weekflag][0]):
        week_money[weekflag][1]+=days_money[i][1]*100
        week_money[weekflag][2] += days_money[i][2]*100
    weekflag =0
...
df1 =pd.DataFrame(arr,columns=['周数','生鲜销售额','一般商品销售额'])
df1.to_csv(r"C:\Users\ASUS\Desktop\技能赛\task1_4.csv",encoding='gbk')
